...
# Monkeypatch: frontier_exploration's _draw_target_bbox_mask assumes integer
# object_id, but COIN goals use string IDs (e.g. "mirror_57"). Skip when
# object_id is not usable as a list index.
try:
    from frontier_exploration.measurements import FrontierExplorationMap as _FEM
    _orig_draw = _FEM._draw_target_bbox_mask

    def _safe_draw_target_bbox_mask(self, episode):
        from habitat.tasks.nav.object_nav_task import ObjectGoalNavEpisode
        if isinstance(episode, ObjectGoalNavEpisode):
            for goal in episode.goals:
                if not isinstance(getattr(goal, "object_id", None), int):
                    return
        return _orig_draw(self, episode)

    _FEM._draw_target_bbox_mask = _safe_draw_target_bbox_mask
except Exception:
    pass
