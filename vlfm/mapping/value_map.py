...
_vm_cfg = load_config().get("value_map", {})
RECORDING = _vm_cfg.get("record", False)
PLAYING = _vm_cfg.get("play", False)
...
class ValueMap(BaseMap):
    ...
    def __init__(
        ...
        _ft = load_config().get("value_map", {}).get("fusion_type", "")
        if _ft:
            self._fusion_type = _ft
