...
class HabitatMixin:
    """Mixin that endows a BaseObjectNavPolicy subclass with the methods
    needed to run inside the Habitat simulator."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # COIN benchmark populates this via set_obj_id_to_name()
        self._category_to_task_category_id: Dict[str, int] = {}

    def set_obj_id_to_name(self, integer_to_string_mapping: Dict[str, int]) -> None:
    ...
    def act(
        ...
        if self._dataset_type == "hm3d":
            if benchmark == "coin" and self._category_to_task_category_id:
                object_id: int = observations[ObjectGoalSensor.cls_uuid][0].item()
                reversed_id_to_name = {
                    obj_id: obj_name
                    for obj_name, obj_id in self._category_to_task_category_id.items()
                }
                obs_dict[ObjectGoalSensor.cls_uuid] = reversed_id_to_name[object_id]
            obs_dict["Goal Attributes"] = goal_attributes
