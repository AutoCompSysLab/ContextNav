...
class ActionReplayPolicy(BasePolicy):
    ...
    def act(
        ...
        img_dir = os.path.join(self._dir, "imgs")
        rgb_path = os.path.join(img_dir, f"{self.curr_idx:05d}_rgb.png")
        depth_path = os.path.join(img_dir, f"{self.curr_idx:05d}_depth.png")

        bgr = cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR)
        cv2.imwrite(rgb_path, bgr)
        cv2.imwrite(depth_path, (depth * 255).astype("uint8"))

        # Log position and yaw to CSV
        ...
        filepath = os.path.join(self._dir, "position_yaws.csv")
        ...
        camera_position = np.array([x, -y, self._camera_height])  # flip y (Habitat convention)
        ...
        frontier_map_path = os.path.join(self._dir, "imgs", f"{self.curr_idx:05d}_frontier_map.png")
