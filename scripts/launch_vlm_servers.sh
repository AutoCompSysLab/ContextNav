...
export GROUNDING_DINO_PORT=${GROUNDING_DINO_PORT:-12220}
export GOAL_CLIP_PORT=${GOAL_CLIP_PORT:-12221}
export SAM_PORT=${SAM_PORT:-12222}
export YOLOV7_PORT=${YOLOV7_PORT:-12223}

#export BLIP2ITM_PORT=${BLIP2ITM_PORT:-12182}
export GOAL_CLIP_CKPT=${GOAL_CLIP_CKPT:-data/GOAL_ViT_large14_DOCCI.pth}
export CUDA_VISIBLE_DEVICES_TMUX=0

session_name=vlm_servers_${RANDOM}
# Create a detached tmux session
tmux new-session -d -s ${session_name}
tmux setenv -t ${session_name} CUDA_VISIBLE_DEVICES ${CUDA_VISIBLE_DEVICES_TMUX}
...
tmux send-keys -t ${session_name}:0.1 "${VLFM_PYTHON} -m vlfm.vlm.grounding_dino --port ${GROUNDING_DINO_PORT}" C-m
#tmux send-keys -t ${session_name}:0.2 "${VLFM_PYTHON} -m vlfm.vlm.blip2itm --port ${BLIP2ITM_PORT}" C-m
tmux send-keys -t ${session_name}:0.2 "${VLFM_PYTHON} -m vlfm.vlm.goal_clip_server --ckpt ${GOAL_CLIP_CKPT} --port ${GOAL_CLIP_PORT}" C-m
tmux send-keys -t ${session_name}:0.3 "${VLFM_PYTHON} -m vlfm.vlm.sam --port ${SAM_PORT}" C-m
tmux send-keys -t ${session_name}:0.4 "${VLFM_PYTHON} -m vlfm.vlm.yolov7 --port ${YOLOV7_PORT}" C-m
